...
def create_pickle_models(hasPolicy, hasContact):
    ...
    df_training["remainingamount"] = pd.Series(
        df_training['remainingamount']).astype(float)
...
def create_comp(hasPolicy, hasContact):
    ...
    if hasPolicy:
        comp.string('policyno', 'policyno',
                    method="jarowinkler", label="policyno")
    if hasContact:
        comp.string('contact', 'contact',
                    method="jarowinkler",  label="contact")
...
def clean_prefixes_and_symbols_from_name(df, colName):
    ...
    title_without_dot_regex = r'^\b(?:' + \
        '|'.join(prefixes) + r')\s*'  # mr, ms
...
def getAIResult(JsonChequeDetails):
    ...
    # Clean cheque data
    if df.iloc[0]["policyno"] is None:
        df["policyno"] = ""
    if df.iloc[0]["contact"] is None:
        df["contact"] = ""
    df["policyno"] = clean(df["policyno"].astype(str))
    df["contact"] = clean(df["contact"].astype(str))
    ...
    customer_data.duedate = customer_data.duedate.str.replace(
        '-', '/', regex=True)
    ...
    customer_data_df["contact"] = clean(
        customer_data_df["contact"].astype(str))
    customer_data_df["policyno"] = clean(
        customer_data_df["policyno"].astype(str))
    customer_data_df['remainingamount'] = pd.Series(
        customer_data_df['remainingamount']).astype(float)
    customer_data_df = customer_data_df.drop(
        columns=['nric', 'policytype', 'duedate'])
    ...
    if hasPolicy & hasContact:
        scores_svm = np.average(
            comparison_vectors_svm.values, axis=1, weights=[10, 30, 30, 30])
    elif hasPolicy:
        scores_svm = np.average(
            comparison_vectors_svm.values, axis=1, weights=[10, 10, 10])
    elif hasContact:
        scores_svm = np.average(
            comparison_vectors_svm.values, axis=1, weights=[10, 10, 10])
    else:  # no contact no policy
        scores_svm = np.average(
            comparison_vectors_svm.values, axis=1, weights=[50, 50])

    # Find svm predicted rows & Add their scores to customer_data df
    scored_comparison_vectors_svm = comparison_vectors_svm.assign(
        score=scores_svm)
    ...
    customer_data_scored_svm = customer_data_scored_svm.assign(
        score=scores_svm)
    customer_data_scored_svm = customer_data_scored_svm.sort_values(
        by='score', ascending=False)

    # Convert df to json
    customer_data_scored_svm_dict = customer_data_scored_svm.to_dict(
        orient='records')
    json_data_dict = json.loads(JsonChequeDetails)["chequeDetail"]
    prediction_result = {"chequeDetail": json_data_dict,
                         "prediction": customer_data_scored_svm_dict}
    prediction_result = json.dumps(prediction_result, indent=4)

    print("predicted results:")
    print(customer_data_scored_svm_dict)
