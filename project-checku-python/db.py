...
def getProccessedCheques(mainStatus):
    if mainStatus == "proccessReject":
        ...
        query = "Select p.PaymentID as paymentId, c.ChequeSignature as signatureExists, c.Date as date, c.Addressee as addressee, c.BankNo as bankNo, c.BankBrNo as branchNo, c.ChequeNo as chequeNo, p.PolicyNo as policyNo, p.PremiumType as premiumType, c.ChequeName as customerName, c.AccountNo as accountNo, c.Contact as contact, c.Amount as amount, c.Date as date, c.ChequeImgFront as imageFront,c.ChequeImgBack as imageBack from PaymentRecord p, Cheque c Where p.ChequeId=c.ChequeId And p.Status='holdingPending' And p.ReviewedAt IS NULL"
    df = pd.read_sql(query, createSqlConn())
    returnedJson = {"data": []}
